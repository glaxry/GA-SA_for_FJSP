...
def decode_operation_index(op_index, parameters):
    """
    将操作序列中的索引映射回其在作业列表中的位置。

    :param op_index: 操作序列中的索引。
    :param parameters: 包含作业和机器信息的参数字典。
    :return: 一个元组 (job_idx, op_idx_within_job)，表示操作所在的作业及其在该作业中的索引。
    """
    total_ops = 0
    for job_idx, job in enumerate(parameters['jobs']):
        total_ops += len(job)
        if op_index < total_ops:
            op_idx_within_job = op_index - (total_ops - len(job))
            return job_idx, op_idx_within_job
    raise ValueError("操作索引超出范围")
